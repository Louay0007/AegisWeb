...
export async function POST(request: NextRequest) {
  const response = await apiFetch("/auth/mfa/challenge", { method: "POST", body: await request.text(), headers: { "content-type": "application/json" } });
  if (!response.ok) return copyResponse(response);
  const parsed = await parseAuthResponse(response);
  if (!parsed?.data.user) return NextResponse.json({ error: { code: "AUTH_FAILED", message: "MFA verification failed." } }, { status: 502 });
